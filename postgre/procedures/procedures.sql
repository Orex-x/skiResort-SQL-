...
language plpgsql
as $$
	DECLARE have_record int := count(*) from Outfit
	where Title = p_Title;
begin
...
create or replace procedure Rental_Outfit_Insert ( p_Price int, p_Date_Issue timestamp, p_Return_Date timestamp, p_Outfit_ID int )
language plpgsql
as $$
...
create or replace procedure Rental_Outfit_Update(p_ID_Rental_Outfit int, p_Price int, p_Date_Issue timestamp, p_Return_Date timestamp, p_Outfit_ID int)
language plpgsql
as $$
	DECLARE have_record int := count(*) from Rental_Outfit
	where Price = p_Price and Date_Issue = p_Date_Issue and Return_Date = p_Return_Date and Outfit_ID = p_Outfit_ID;
...
language plpgsql
as $$
	DECLARE have_record int := count(*) from Status_Card
	where Title = p_Title;
begin
		if have_record > 0 then
...
create or replace procedure Bonus_Card_Insert ( p_Balance int, p_Tariff varchar(50), p_Status_Card_ID int )
language plpgsql
as $$
...
create or replace procedure Bonus_Card_Update(p_ID_Bonus_Card int, p_Balance int, p_Tariff varchar(50), p_Status_Card_ID int)
language plpgsql
as $$
	DECLARE have_record int := count(*) from Bonus_Card
	where Balance = p_Balance and Tariff = p_Tariff and Status_Card_ID = p_Status_Card_ID;
...
create or replace procedure Employee_Insert ( p_First_Name varchar (30), p_Second_Name varchar (30), p_Middle_Name varchar (30), p_Phone_Number varchar(19),
...
create or replace procedure Employee_Update(p_ID_Employee int, p_First_Name varchar (30), p_Second_Name varchar (30), p_Middle_Name varchar (30), p_Phone_Number varchar(19),
											 p_Email varchar, p_Age int, p_INN varchar(12), p_SNILS varchar(14)
											 ,p_Post_Part decimal (38,1), p_Position_ID int)
language plpgsql
as $$
...
create or replace procedure Track_Update(p_ID_Track int, p_Complexity int, p_Title varchar(50), p_Employee_ID int, p_Cableway_ID int )
language plpgsql
as $$
	DECLARE have_record int := count(*) from Track
	where Complexity = p_Complexity and Title = p_Title and Employee_ID = p_Employee_ID and Cableway_ID = p_Cableway_ID;
...
create or replace procedure Status_Card_Track_Update(p_ID_Status_Card_Track int,  p_Track_Permission boolean, p_Track_ID int, p_Status_Card_ID int )
language plpgsql
as $$
	DECLARE have_record int := count(*) from Track
	where Track_Permission = p_Track_Permission and Track_ID = p_Track_ID and Status_Card_ID = p_Status_Card_ID;
...
create or replace procedure Client_Insert ( p_First_Name varchar (30), p_Second_Name varchar (30), p_Middle_Name varchar (30), p_Phone_Number varchar(19),
...
create or replace procedure Client_Update(p_ID_Client int, p_First_Name varchar (30), p_Second_Name varchar (30), p_Middle_Name varchar (30), p_Phone_Number varchar(19),
											 p_Email varchar, p_Age int, p_NumberDL varchar(11), p_Bonus_Card_ID int)
language plpgsql
as $$
	DECLARE have_record int=count(*) from Client
...
create or replace procedure Receipt_Update(p_ID_Receipt int, p_Date_Of_Creation date, p_Employee_ID int, p_Rental_Outfit_Client_ID int )
language plpgsql
as $$
	DECLARE have_record int := count(*) from Receipt
	where Date_Of_Creation = p_Date_Of_Creation and Employee_ID = p_Employee_ID and Rental_Outfit_Client_ID = p_Rental_Outfit_Client_ID;
...
create or replace procedure Rental_Outfit_Client_Update(p_ID_Rental_Outfit_Client int,  p_Employee_ID int, p_Client_ID int, p_Rental_Outfit_ID int )
language plpgsql
as $$
	DECLARE have_record int := count(*) from Rental_Outfit_Client
	where Employee_ID = p_Employee_ID and Client_ID = p_Client_ID and Rental_Outfit_ID = p_Rental_Outfit_ID;
