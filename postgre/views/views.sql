...
as  select
        concat(Second_Name, ' ', substring(First_Name, 1,1),'.',substring(Middle_Name, 1,1),'.') as "Фамилия и инициалы сотрудника",
        concat('Мобильный телефон: ', Phone_Number, ', адрес электронной почты: ', Email) as "Контактная информация",
        concat
        (
            'Название должности - ', Title,'с окладом в ',cast(Post_Price as varchar(100)),'руб., по ставке - ',Post_Part,
...
from Employee inner join Position on Position_ID = ID_Position;



create or replace view View_Client
as  select
               concat(Second_Name, ' ', substring(First_Name, 1,1),'.',substring(Middle_Name, 1,1),'.') as "Фамилия и инициалы клиента",
               concat('Мобильный телефон: ', Phone_Number, ', адрес электронной почты: ', Email) as "Контактная информация",
               concat('Статус карты: ', Status_Card.Title, 'Количество баллов: ', Bonus_Card.Balance) as "Данные о карте клиента"
    from Client
...
as  select
               concat(Employee.Second_Name, ' ', substring(Employee.First_Name, 1,1),'.',substring(Employee.Middle_Name, 1,1),'.') as "Фамилия и инициалы сотрудника",
               concat('Мобильный телефон: ', Employee.Phone_Number, ', адрес электронной почты: ', Employee.Email) as "Контактная информация сотрудника",
               concat(Client.Second_Name, ' ', substring(Client.First_Name, 1,1),'.',substring(Client.Middle_Name, 1,1),'.') as "Фамилия и инициалы клиента",
               ...
               concat('Цена за услугу аренды: ', Rental_Outfit.Price, 'Дата выдачи снаряжения: ',
                      ...
                      'Наименование снаряжения: ', Outfit.Title) as "Данные об арденде"
